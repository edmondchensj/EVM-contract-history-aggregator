from HistoricalTable import HistoricalTable
...
def make_database(trace_dir):
    ...
    addresses = [item for item in os.listdir(trace_dir)
            if os.path.isdir(os.path.join(trace_dir, item))]
    total_addresses = len(addresses) + 1

    for address in tqdm(addresses, desc='Progress'):
